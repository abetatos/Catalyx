...
_REBALANCE_FIELDS = (
    "sector_id", "etf", "rank", "target_pct", "actual_pct", "gap_eur", "rule_action", "trade_eur",
    "tax_eur", "breakeven_pct", "reason", "regime_state", "exit_action", "flags",
)
...
def build(as_of: str | None = None, top_n: int = 15, lake_dir: Path | None = None) -> dict:
    ...
    return {
        ...
        "rank_moves": _try("rank_moves", lambda: lake_query.rank_moves(top_n=10, lake_dir=lake_dir), []),
        ...
        "exposure": _try("exposure", lambda: lake_query.catalyst_ledger(lake_dir=lake_dir), []),
...
def _swaps_and_partials(reb_rows: list[dict]) -> dict:
    ...
    try:
        ...
        return {
            ...
            "partials": rebalance.partial_rungs(reb_rows, cfg),
...
def render(d: dict) -> str:
    ...
    if drag is not None and drag == drag:
        out.append(f"  CASH DRAG €{float(reb.get('cash_actual_eur') or 0):,.0f} idle since "
                   f"{reb.get('cash_idle_since')} ({int(float(reb.get('cash_idle_days') or 0))}d)"
                   f" · benchmark "
                   f"{float(reb.get('benchmark_return_pct') or 0):+.2f}% → €{float(drag):,.0f} "
                   f"forgone")
