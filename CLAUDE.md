...
|---|---|---|---|
| 2026-06-15 | `data/catalysts/cat_20260228_hormuz_closure.json` + `data/catalysts/cat_20260614_hormuz_deescalation_deal.json` (new) + `experiments/exp_2026-06-15_hormuz_deescalation.md` (new) + `experiments/README.md` | v0.5.2 | **Iran/Hormuz de-escalation catalyst + pre-open prediction experiment.** The 2026-06-14 US-Iran deal to reopen the Strait of Hormuz (MoU 19-Jun, 60-day ceasefire; Brent −4.7%) is the opposite-sign reversal of `cat_20260228_hormuz_closure`. Registered the reversal as its OWN event `cat_20260614_hormuz_deescalation_deal` (`relation_to_structural: contradicts`, strength 68, is_priced_in 0.50, halflife 30) rather than mutating the closure. **Closure left intact** — `strength_score` kept at 94 (historical impact is a record, not a live dial; decay handles the time-fade → LNG `catalyst_alignment` ≈ 27 today; `status` flips to `invalidated` only when transit is physically restored). `catalyst_scorer` does NOT aggregate a "direct" contradicts event, so energy alignment falls via natural decay, not a hand-edited number. Logged a **falsifiable pre-open prediction** (markets closed, no look-ahead): oil/LNG/tankers DOWN, broad market + oil-importers (India) risk-ON — graded at the next `/catalyx-review scheduled` after market open (🟡 OPEN experiment indexed in `experiments/README.md`). _(Process note: an earlier draft wrongly cut the closure's strength 94→45 and mixed the reversal into its `related_catalyst_ids`; reverted — captured in memory `feedback-catalyst-strength-vs-decay`.)_ |
