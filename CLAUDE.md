...
| Lake query | `catalyx/store/lake_query.py` | **Fase E.** Unified DuckDB read-path over the lake — the day-to-day query layer and the data foundation for the GitHub-Pages dashboard (DuckDB-WASM runs the same SQL in-browser). Read-only, defensive (empty table → empty result). `sector_history` / `latest_ranking` / `rank_moves` / `portfolio_compare` / `portfolio_holdings` / `catalyst_ledger` / `lineage_for_movement` (movement → catalysts → run → reports+snapshot) / `catalyst_lineage` (catalyst → real movements + driven sectors + per-strategy exposure **time-series**: each portfolio rebalance is a point, with a `combined` = mean exposure across strategies = the system's conviction in the catalyst) / `sql`. CLI: `… lake_query {ranking,sector,moves,portfolios,holdings,ledger,lineage,catalyst-lineage,sql}`. `snapshot_repo` read queries (history/runs/events) now read the lake too. |
