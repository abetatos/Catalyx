...
| Model portfolios | `catalyx/execution/portfolio.py` | **Fase D.** Deterministic model portfolios = `(score_run × risk_config)`. Reads lake `sector_snapshot`, applies a profile's construction rules (filter → dedupe-by-ETF → top-N → water-fill cap), writes `portfolio_holding` (partitioned by portfolio_id+run_id) tagged with `config_version`. Profiles in `config/portfolios/*.yaml` (conservative/balanced/aggressive). CLI: `uv run python -m catalyx.execution.portfolio {profiles,build,build-all,show}`. |
| NAV engine | `catalyx/execution/nav_engine.py` | **Fase D.2.** Buy-and-hold NAV time series (indexed to 100) from holdings — model OR real — vs benchmark. `holdings_nav` (price source injectable; yfinance default), `compute_model_nav` / `compute_real_nav` → lake `portfolio_nav` (one series file per portfolio). CLI: `… nav_engine {model,real,show} <portfolio_id>`. |
