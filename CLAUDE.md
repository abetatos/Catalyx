...
| Exit watcher | `scorer/exit_watcher.py` | Sell-signal Family 1: evaluates `risk_discipline.invalidation[]` stops deterministically + assumptions + regime + after-tax P&L → Exit/Reduce/Watch/Hold. Recommend-only, persists `exit_signal`. `docs/DESIGN_sell_signals.md`. CLI `[--json] [--no-persist]` |
