...
tests/  conftest.py (runtime-env isolation) + unit/ 667 tests · docs/ DESIGN_*/PLAN_*/MODULES.md
...
**That check counts the SELLS that fund the buys** (v9): it used to skip every negative row, so a
table closing a position to buy inside the same cluster was measured as if the sale never happened
— live, that read 39.1% (breach) against 29.1% (compliant) on the same table. The deduction is
capped at the exposure BASIS: a winner sold above cost cannot retire more exposure than it carried.
...
**The ratio says WHERE the book belongs; the ramp says how fast it gets there (v9 R1).**
`deployment.ramp.max_step_pp` (**15pp** of total capital) is the most one review may raise the
DEPLOYED share, so an under-deployed book stops being asked to close the whole gap at one
afternoon's prices — the largest timing bet the system can make and the one nobody chose. It caps
the NET, so a rotation passes untouched; it fills in RANK order to FULL size, because six
quarter-positions cost six slots and six spreads to buy a quarter of the exposure; and it
**never deletes the breach** — the shortfall is still measured against the full `deployable`, the
cash drag still prints beside it, and every queued row is logged (author `ramp`) and priced at 21
days, so the schedule itself is falsifiable. Order matters: **cash is rationed before slots**
(ramp, then `trade_budget_plan`) — a slot spent on a row the ramp will queue is spent on nothing.
The table's *destination* and its *order ticket* are different questions and are printed as such:
`THIS ITERATION` lists what to place today, a trailing `*` marks what a scarcity queued.
**Neither scarcity ever queues a full exit.** The ramp grants every negative row whatever its
size (selling deploys nothing), and the slot budget exempts `SELL`/`REDUCE` *and* any row that
closes the line — `is_full_exit()` + `exempt_full_exits`, because a full exit arrives labelled
TRIM whenever the model target is 0%, which is how every exit from a sector the book no longer
holds arrives. A TRIM against a LIVE target is still turnover and still starves first.
...
| Date | Version | Change |
|---|---|---|
| 2026-09-01 | v9.0 | **La regla decía DÓNDE debe estar el libro y nunca dijo A QUÉ VELOCIDAD — más el cap que ignoraba las ventas que lo financian.** Salió de dos frases del usuario: «pone 773 hold y yo tengo 565 en uspy» y «no meter los 10.000 de golpe, ir escalando». **El 773 no era un defecto: era el `target_eur` de otro run** (`run_20260828_102925`), cuya fila `actual_eur` decía 564,57 — los €565 del bróker exactos. La columna «actual» siempre fue correcta; lo que confunde es que la tabla contesta DOS preguntas en columnas contiguas (a dónde va el libro / qué se ejecuta hoy). Eso lo arregla el bloque **THIS ITERATION**: las filas que sobreviven a las dos escaseces, en orden de ejecución, arriba del todo. **La rampa (`deployment.ramp.max_step_pp: 15`)** es la mitad que le faltaba al `deploy_ratio`: el destino no cambia, la RUTA son 15pp de capital por review. Limita el NETO (una rotación pura pasa entera), rellena por RANK y a tamaño COMPLETO (un cuarto de posición cuesta el mismo hueco y el mismo spread por un cuarto de exposición), y **no borra el incumplimiento**: el shortfall se sigue midiendo contra el `deployable` completo y cada fila encolada se registra (autor `ramp`) y se precia a 21 días. **Tres defectos que destapó, que valen más que la función:** (1) el presupuesto de operaciones racionaba huecos entre filas que la rampa iba a encolar — seis BUYs echaban de la tabla el TRIM de pharma que el usuario ya había aprobado; ahora CAJA primero, HUECOS después, y el tramo pasa de 3 filas a 5. (2) **`cap_check` ignoraba las ventas que financian las compras** (`if eur <= 0: continue`), así que una tabla que cierra copper para comprar en el MISMO clúster se medía como si la venta no ocurriera: `struct_ai_capex_supercycle` leía **39,1% (incumple)** contando el cobre que se vende y **29,1% (cumple)** contándolo bien — la decisión entera. La deducción se limita a la BASE de exposición. (3) Re-correr el rebalance duplicaba los DEFER automáticos y dejaba vivos los de un corte anterior: ahora se reconcilian contra la tabla vigente y nunca se archiva un segundo veredicto sobre una fila que una PERSONA ya contestó. (4) **`TRIM` no estaba exento del presupuesto de huecos**, y una salida completa llega etiquetada `TRIM` siempre que el target es 0% — o sea, toda salida de un sector que el model book ya no tiene: la asignación de huecos podía decidir MANTENER riesgo. `exempt_full_exits` + `is_full_exit()` lo corrigen sin tocar el tier 2 (un TRIM contra un target VIVO sigue siendo rotación y sigue cediendo el hueco primero). 667 tests en verde (+12). |
| 2026-09-01 | v8.1 | **El vehículo que sale en el buscador es el vehículo que se puntúa.** El usuario buscó `IH2O` en el bróker y el ETF no aparece: aparecen **HTO** (H2O America) y **GWRS** (Global Water Resources), dos *utilities* de agua US de **acción única** — el buscador matchea el texto «water», no el vehículo. `RBOT` tampoco resuelve; el fondo sale como **2B76**. La regla v2.0 del universo dice que `chain[0]` es el vehículo REALMENTE comprable, y un ticker que el bróker no devuelve la incumple igual que puntuar `COPX` y comprar `4COP.DE`: no es comprable *de hecho*. Tier-1 pasa a la línea con la que el fondo SÍ aparece — `water_infrastructure` **IH2O.L → IQQQ.DE** (Xetra, EUR) y `robotics_automation` **RBOT.L → 2B76.DE** (Xetra, EUR), mismo fondo y misma clase en ambos casos, identidad verificada contra yfinance (`longName`/`currency`/`exchange`) como exige el universo desde el incidente `IQQR.DE`. Las líneas viejas bajan a tier-2 con la razón escrita, no se borran; ninguna está en cartera, así que no se toca ninguna base de coste. **Lo que además corrige, y no es cosmético:** el comentario de `SECTOR_TICKERS` afirmaba que cotizar en GBp «no da problema porque el momentum es un RETORNO y la escala se cancela». La escala se cancela; **la divisa no**. `IH2O.L` medía el retorno de un comprador en GBP mientras el operador paga en EUR — mismo fondo, mismo día: 1y **−0,09%** en GBp contra **+1,51%** en EUR, 6m −6,41% contra −3,90%. `broker_access` sigue `assumed`: vistos en el buscador, no operados. 663 tests en verde. |
