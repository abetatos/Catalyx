...
tests/  conftest.py (runtime-env isolation) + unit/ 639 tests · docs/ DESIGN_*/PLAN_*/MODULES.md
```
...
- [ ] **Decisión pendiente AHORA (refrescada 2026-08-31 tras el arreglo de flow, v6.9):**
      ejecutar la tabla deja `struct_ai_capex_supercycle` en **38,8%** del nocional contra el
      cap del **30%** — €881 por encima, en `cloud_software_saas` y `robotics_automation`.
      Era 55,4% antes de reparar la fuente de flujo: buena parte del incumplimiento lo estaba
      generando un dato roto, no el libro. `enforcement: warn`, así que el cap no decide: hay
      que bajar tamaño, soltar un nombre, o escribir el `correlation_note`.
...
| 2026-08-31 | v6.7 | **El percentil saturaba justo en los catalizadores que existe para medir.** Plan v6 J4, el ítem más delicado: cambia scores PUBLICADOS. **Diagnóstico (D5):** una serie que sube de forma persistente —compras de oro de bancos centrales, capex hyperscaler— está en su propio máximo casi todos los meses, así que el percentil se clava en ~100 y deja de discriminar; y la primera señal de desaceleración solo llega cuando el valor cae por debajo de lecturas pasadas, que en una serie tendencial es tardísimo. **Pero destendenciar y ya sería el error contrario:** «en máximo histórico» ES información, y puntuar solo el residuo dejaría a un driver en récord en la media por el delito de ir exactamente sobre su propia tendencia. Así que se **mezclan**: el nivel dice cuán fuerte es contra su historia, el residuo si va por encima o por debajo de su trayectoria. Dos catalizadores en récord, uno acelerando y otro aplanándose, ya puntúan distinto — que es el poder discriminante que D5 buscaba. `detrend_weight: 0.0` reproduce el score viejo exacto, y el destendenciado solo se activa si la serie tiene tendencia de verdad (|τ de Kendall| ≥ 0,5, por rangos para que un solo dato enorme no se disfrace de tendencia). **Desviación deliberada del propio plan:** decía «residuo contra una media rolling»; una ventana rolling sobre una serie de 6 puntos tira la mitad de la muestra y no devuelve nada para los primeros — un detrend lineal OLS conserva todas las observaciones, que es la restricción que muerde en indicadores mensuales. **La otra mitad: el acantilado de la 6ª observación.** El score saltaba de golpe de la curva saturante al percentil al llegar el punto `min_history_points`, así que UN dato nuevo movía un indicador decenas de puntos por razones ajenas al mundo. Ahora se mezcla linealmente en `[min−2, min+2]`: n≤4 curva, n=6 mitad y mitad, n≥8 percentil. Misma familia de mini-cliff que v1.5 quitó del semáforo e I6 del freno del VIX. **Un off-by-one real, cazado por no fiarme de mi primera tabla:** la comparación inicial usó `history_blend_span: 0` como base «pre-v6», que es lo que el config prometía. No lo era: en `n == min_history_points` exacto la condición era `n <= lo`, así que con span=0 se tomaba el FALLBACK donde pre-v6 tomaba el percentil. La primera comparación iba contra una base que nunca existió y reportaba el mayor movimiento como −24,8 cuando el real es **+44,0**. Corregido a `<` estricto, y `--compare-legacy` ejecuta el MISMO camino de código con las features a cero en vez de una reimplementación que podría desviarse. **Efecto medido, con la causa atribuida:** 6 de 48 indicadores se mueven, 4 de 13 catalizadores — cyber +9,0 (ambas), oro −6,5 (mezcla de historia), espacio −2,2 (ambas), grid −0,4 (detrend). **El caso instructivo:** ARR YoY de CrowdStrike, historia 0,33→0,29→0,27→0,22→0,24, hoy 0,23, umbrales 0,10/0,20. Tres lecturas legítimas del mismo número: percentil de nivel **25** (cerca del fondo de su rango), curva de umbral **84** (bien por encima de «strong») y percentil destendenciado **83** (τ=−0,73: la serie cae y 0,23 está POR ENCIMA de esa caída, la desaceleración se aplana). Pre-v6 publicaba un 25 pelado y tiraba las otras dos. **Y ojo: el detrend corta en AMBOS sentidos, cosa que el ejemplo motivador esconde.** D5 iba de series que suben clavándose en 100; aquí la serie cae y el residuo SUBE el score. Es simétrico y correcto —el residuo mide posición contra tu propia trayectoria, sea cual sea su signo— pero quien espere «detrend = amortiguador» se llevaría una sorpresa, así que la salida lo dice. El canal a la baja para un catalizador que de verdad muere sigue siendo `trend_delta`, intacto. **Migración:** `catalyst_scorer` lee el `intensity.current_score` ALMACENADO, así que **nada aguas abajo se mueve hasta que alguien corra `--write-back`** — hay un test que lo fija. Inspecciona antes con `intensity_engine --compare-legacy`. 606 tests en verde (+12). |
| 2026-08-31 | v6.6 | **Qué constantes tienen consecuencias — y el harness que casi miente sobre ello.** Plan v6 I4. `experiments/sensitivity_weights.py` perturba cada constante ±25% y ±50%, de una en una, sobre el último run, y mide si el OUTPUT se mueve: τ de Kendall contra el orden base más el Jaccard del top-10 (τ puede quedarse alto mientras la cabeza rota, y la cabeza es la parte que se convierte en posiciones). Es la respuesta barata y honesta a «estos porcentajes son arbitrarios»: no un relato mejor sobre de dónde salió cada uno, sino una medición de cuáles merece la pena discutir. **EL MODO DE FALLO DEL PROPIO HARNESS ES EL FALSO NEGATIVO, y me pilló dos veces antes de que la tabla fuera fiable:** una perilla que no alcanza su objetivo imprime exactamente igual que una constante que no importa — un «INERTE» confiado que en realidad es «no lo he tocado». **(1)** Los pesos de momentum salieron τ=1.000 INERTE, cuando H4 acababa de medir que rebalancear el blend mueve rangos hasta 7 puestos: `momentum_engine` desempaqueta `_MPW` en `_WEIGHT_1M/3M/6M` en el import y ESOS son los que lee; la perilla parcheaba `_MPW` y no llegaba a nada. Arreglado, ahora están entre las filas más consecuentes (τ 0,895/0,938). **(2)** `sharpness` salió 0,00pp: el perfil declara el suyo y gana sobre el global. Misma clase, una capa más abajo. Por eso el harness lleva ahora una **perilla de control** —`z_scale`, un reescalado monótono que DEBE mover todos los scores y no reordenar nada—; si falla cualquiera de las dos mitades la tabla imprime «FAIL — el harness está roto, ignora esta tabla». Una tabla de sensibilidad que nadie puede falsar es decoración. Nueve tests fijan la maquinaria, incluidos los dos falsos negativos como regresiones. **Una tercera distinción que la primera tabla colapsaba:** τ=1.000 reportaba como una sola cosa dos muy distintas — una constante que mueve todos los scores sin reordenar, y otra que el camino vivo nunca consulta. Una columna `moved` las separa, y cada fila NOT REACHED lleva su razón VERIFICADA: el halflife por defecto no se lee nunca (los 15 eventos activos declaran el suyo); `confirm_max` necesita un evento que confirme un estructural del mismo sector, y hoy no hay; y **`intensity_trend_deltas` no se aplica al puntuar** — `catalyst_scorer` lee el `intensity.current_score` ALMACENADO, así que esas constantes solo actúan en el `--write-back`. Ese último es un hecho de arquitectura que conviene saber por sí solo. **Y el hallazgo por el que valía la pena construirlo:** `sharpness` mueve **0,00pp en vivo y 2,78pp con λ=1**. No es inerte, está **APAGADA**: `skill_shrink` colapsa la pata de convicción hacia un libro neutral y hoy λ=0 (el IC medido es ruido). Archivar «desactivada por una medición» bajo «no importa» sería la conclusión contraria, así que las filas de sizing se miden dos veces y llevan ambas columnas. `vol_tilt_alpha` da 2,40 en vivo contra 2,37 sin shrinkage — casi idéntico, que es el diseño documentado confirmándose: `vol_tilt` corre DESPUÉS del shrinkage precisamente para que neutral signifique neutral en RIESGO. **Lo que dice la tabla de ranking:** los cuatro pesos del compuesto son las filas más consecuentes (τ 0,858–0,938, Jaccard top-10 0,818 — un empujón del ±50% en cualquiera rota dos de diez nombres), luego los periodos de momentum, luego `reinforce_factor` (τ 0,994). Nada es DECISIVO según el propio umbral del harness. Léase como: la cadena no está en equilibrio sobre un filo, y la evidencia se debe primero sobre los pesos del compuesto — que es exactamente lo que la fase K está esperando en datos. **Una constante INERTE no es por ello una constante EQUIVOCADA:** es indiscutible sobre el universo de hoy, y el hallazgo puede no sobrevivir a uno con otra forma. La salida lo dice. 594 tests en verde (+9). |
